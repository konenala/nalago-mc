//go:build ignore
// +build ignore

// Code generated by enhanced-generator v2 from protocol.json; DO NOT EDIT manually.
// To regenerate: go run main_v2.go -protocol <path> -output <dir> -direction server

package server

import (
	pk "git.konjactw.dev/falloutBot/go-mc/net/packet"
	"git.konjactw.dev/patyhank/minego/pkg/protocol/packetid"
	"io"
)

// PingRequest represents the Serverbound PingRequest packet.

type PingRequest struct {
	Id int64
}

// PacketID returns the packet ID for this packet.
func (*PingRequest) PacketID() packetid.ServerboundPacketID {
	return packetid.ServerboundPingRequest
}

// ReadFrom reads the packet data from the reader.
func (p *PingRequest) ReadFrom(r io.Reader) (n int64, err error) {
	var temp int64
	_ = temp

	temp, err = (*pk.Long)(&p.Id).ReadFrom(r)
	n += temp
	if err != nil {
		return n, err
	}

	return n, nil
}

// WriteTo writes the packet data to the writer.
func (p PingRequest) WriteTo(w io.Writer) (n int64, err error) {
	var temp int64
	_ = temp

	temp, err = pk.Long(p.Id).WriteTo(w)
	n += temp
	if err != nil {
		return n, err
	}

	return n, nil
}

func init() {
	registerPacket(packetid.ServerboundPingRequest, func() ServerboundPacket {
		return &PingRequest{}
	})
}

//go:build ignore
// +build ignore

// Code generated by enhanced-generator v2 from protocol.json; DO NOT EDIT manually.
// To regenerate: go run main_v2.go -protocol <path> -output <dir> -direction server

package server

import (
	pk "git.konjactw.dev/falloutBot/go-mc/net/packet"
	"git.konjactw.dev/patyhank/minego/pkg/protocol/packetid"
	"io"
)

// LockDifficulty represents the Serverbound LockDifficulty packet.

type LockDifficulty struct {
	Locked bool
}

// PacketID returns the packet ID for this packet.
func (*LockDifficulty) PacketID() packetid.ServerboundPacketID {
	return packetid.ServerboundLockDifficulty
}

// ReadFrom reads the packet data from the reader.
func (p *LockDifficulty) ReadFrom(r io.Reader) (n int64, err error) {
	var temp int64
	_ = temp

	var locked pk.Boolean
	temp, err = locked.ReadFrom(r)
	n += temp
	if err != nil {
		return n, err
	}
	p.Locked = bool(locked)

	return n, nil
}

// WriteTo writes the packet data to the writer.
func (p LockDifficulty) WriteTo(w io.Writer) (n int64, err error) {
	var temp int64
	_ = temp

	temp, err = pk.Boolean(p.Locked).WriteTo(w)
	n += temp
	if err != nil {
		return n, err
	}

	return n, nil
}

func init() {
	registerPacket(packetid.ServerboundLockDifficulty, func() ServerboundPacket {
		return &LockDifficulty{}
	})
}
